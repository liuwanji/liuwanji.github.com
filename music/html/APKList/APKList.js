...
function addlisten() {
	...
	$("#create_apk").on("click", function() {
		...
		$("#apklistMain").animate({top: '30%',opacity: '1'}, 400);
	});
	/*新增渠道弹窗中关闭按钮的点击事件*/
	...
	/*编辑的点击事件*/
	$("#apklistBody").on("click",".apk_bianji",function() {
		addoredit = 2;
    ...
    ajax_public(url, function(msg) {
        ...
        if(msg.status == 0){
	        ...
	        $.each(msg.data.info,function (index,element) {
	        	apk += apkStr.replace("%autoid",element.autoid).replace("%apkid",element.apkid).replace("%name",element.name).
	        	replace("%remarks",element.remarks);
	        });
	        $("#all_num,#countNum").html(msg.data.total);
	        $('#apklistBody').html(apk);
	        if(type == 1) {
				changePage();
			}
        }else{
